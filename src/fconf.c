...
static int set_char(struct fconf *conf, int vidx, char val)
{
	char *p;
		   ...
		   const char *val)
{
	switch (ck->typ) {
		...
		case CONF_CHAR:
			return set_char(conf, ck->vidx, val[0]);
		default:
			return EINVAL;
	}
}
...
static char get_char(const struct fconf *conf, int vidx)
{
	char v = *((char *) (((uint8_t *) conf) + vidx));
...
static int write_template(struct fconf *conf, const char *fname)
{
	...
	for (size_t i = 0; i < n; ++i) {
		...
		switch (ck->typ) {
			...
			break;
			case CONF_CHAR:
				err = set_char(conf, ck->vidx, ck->defint);
			break;
			default:
				return EINVAL;
		}
